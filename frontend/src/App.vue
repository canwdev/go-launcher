...
<template>
  <div class="flex h-screen flex-col bg-gray-100 text-sm text-gray-800 dark:bg-gray-900 dark:text-gray-100">
        ...
        <Menu as="div" class="relative">
          ...
          <TransitionRoot enter="transition duration-100 ease-out" enter-from="opacity-0 scale-95"
            ...
            <MenuItems
              ...
              <MenuItem v-slot="{ active }">
                <button class="flex w-full items-center gap-2 px-3 py-1.5 text-left"
                  :class="active ? 'bg-gray-100 dark:bg-gray-700' : ''" @click="onBatchUpdateIcons">
                  <span
                    class="inline-flex h-4 w-4 shrink-0 items-center justify-center text-gray-500 dark:text-gray-400">
                    🖼
                  </span>
    ...
    <AppDialog :open="modalOpen" :title="modalTitle" @close="closeModal">
      <input v-model="modalName" type="text" autofocus
    ...
    <AppDialog :open="confirmOpen" title="Confirm" @close="closeConfirm">
      ...
      <template #actions>
        ...
        <button class="rounded border border-red-600 bg-red-500 px-2.5 py-1 text-white hover:bg-red-600"
          @click="onConfirm">
          Delete
        </button>
      </template>
    ...
    <div class="pointer-events-none fixed bottom-3 left-1/2 z-50 flex -translate-x-1/2 flex-col items-center gap-2">
      <TransitionGroup name="toast">
        <div v-for="toast in toasts" :key="toast.id" class="rounded border px-3 py-1.5 text-sm shadow-md"
